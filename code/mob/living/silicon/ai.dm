...
/mob/living/silicon/ai/show_message(msg, type, alt, alt_type, group = 0, just_maptext, var/image/chat_maptext/assoc_maptext = null, var/image/chat_maptext/assoc_deaf_maptext = null)
	..()
...
/mob/living/silicon/ai/emote(var/act, var/voluntary = 0, var/emoteTarget = null, datum/emote/actual_emote, param = null)
	if(voluntary && !src.emote_allowed)
		return

	var/datum/emote/the_datum =null

	var/what_to_do = AI_emotes.Find(lowertext(act))
	if (what_to_do)
		the_datum = get_singleton(AI_emotes[lowertext(act)])
	else
