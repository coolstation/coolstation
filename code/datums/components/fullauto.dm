...
/datum/component/holdertargeting/fullauto/proc/begin_shootloop(mob/living/user, object, location, control, params)
	if(!stopping)
		...
		RegisterSignal(user, COMSIG_MOUSEUP, PROC_REF(end_shootloop))
...
/datum/component/holdertargeting/fullauto/proc/shootloop(mob/living/L)
	...
	delay = delaystart
	while(!stopping)
		G.Shoot(target ? target : get_step(L, NORTH), get_turf(L), L, src.target_pox, src.target_poy)
...
TYPEINFO(/datum/component/holdertargeting/fullauto/callback)
	initialization_args = list(
		ARG_INFO("delaystart", "num", "Initial delay between shots (in deciseconds)", 1.5),
		ARG_INFO("delay_callback", "ref", "ref to a callback datum that will determine next shot delay", null),
	)
