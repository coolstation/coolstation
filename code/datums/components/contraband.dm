TYPEINFO(/datum/component/contraband)
	initialization_args = list(
		ARG_INFO("contraband_level", "num", "Non-firearm Contraband level this item will have", 0),
		ARG_INFO("carry_level", "num", "Firearm Contraband level this item will have", 0),
	)
...
/datum/component/contraband/Initialize(contraband_level = 0, carry_level = 0)
	...
	if (isitem(AM))
		...
		RegisterSignal(AM, list(COMSIG_ITEM_UNEQUIPPED, COMSIG_ITEM_DROPPED), PROC_REF(removed))
...
/datum/component/contraband/proc/contraband_logic(atom/movable/owner, atom/movable/location, multiplier = 1)
	...
	if(contra_guns)
		APPLY_ATOM_PROPERTY(location, PROP_MOVABLE_VISIBLE_GUNS, src, contra_guns * multiplier)

	var/contra_nonguns = GET_ATOM_PROPERTY(owner,PROP_MOVABLE_VISIBLE_CONTRABAND)
	if(contra_nonguns)
		APPLY_ATOM_PROPERTY(location, PROP_MOVABLE_VISIBLE_CONTRABAND, src, contra_nonguns * multiplier)
