//Thank you to TobleroneSwordfish of Goonstation for this component!
		...
		switch (M.dir)
			...
			if(WEST)
				if(M.hand == 1)
					...
					handset_offset_y = -4
					handset_offset_x = -4
	var/datum/lineResult/result = drawLine(src.parent, get_turf(target), src.cord_line, src.cord_cap, src.parent_atom.pixel_x + src.base_offset_x, src.parent_atom.pixel_y + src.base_offset_y, target.pixel_x + handset_offset_x, target.pixel_y + handset_offset_y, LINEMODE_STRETCH_NO_CLIP, applyTransform = FALSE)
