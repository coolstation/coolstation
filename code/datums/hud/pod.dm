/datum/hud/pod
	...
	New(P)
		..()
		master = P
		missing = image('icons/ui/hud_pod.dmi', "marker")
		engine = create_screen("engine", "Engine", 'icons/ui/hud_pod.dmi', "engine-off", "NORTH-1,WEST", tooltipTheme = "pod-alt", desc = "Turn the pod's engine on or off (you probably don't want to turn it off)")
		wormhole = create_screen("wormhole", "Create Wormhole", 'icons/ui/hud_pod.dmi', "wormhole", "NORTH-1,WEST+1", tooltipTheme = "pod", desc = "Open a wormhole to a beacon that you can fly through")
		life_support = create_screen("life_support", "Life Support", 'icons/ui/hud_pod.dmi', "life_support-off", "NORTH-1,WEST+2", tooltipTheme = "pod-alt", desc = "Turn life support on or off")
		comms = create_screen("comms", "Comms", 'icons/ui/hud_pod.dmi', "comms-off", "NORTH-1,WEST+3", tooltipTheme = "pod-alt", desc = "Turn the pod's communications system on or off")
		use_comms = create_screen("comms_system", "Use Comms System", 'icons/ui/hud_pod.dmi', "comms_system", "NORTH-1,WEST+4", tooltipTheme = "pod", desc = "Use the communications system to talk or whatever")
		sensors = create_screen("sensors", "Sensors", 'icons/ui/hud_pod.dmi', "sensors-off", "NORTH-1,WEST+5", tooltipTheme = "pod-alt", desc = "Turn the pod's sensors on or off")
		sensors_use = create_screen("sensors_use", "Activate Sensors", 'icons/ui/hud_pod.dmi', "sensors-use", "NORTH-1,WEST+6", tooltipTheme = "pod", desc = "Use the pod's sensors to search for drones and lifeforms nearby")
		weapon = create_screen("weapon", "Main Weapon", 'icons/ui/hud_pod.dmi', "weapon-off", "NORTH-1,WEST+7", tooltipTheme = "pod-alt", desc = "Turn the main weapon on or off, if the pod is equipped with one")
		lights = create_screen("lights", "Toggle Lights", 'icons/ui/hud_pod.dmi', "lights_off", "NORTH-1, WEST+8", tooltipTheme = "pod", desc = "Turn the pod's external lights on or off")
		secondary = create_screen("secondary", "Secondary System", 'icons/ui/hud_pod.dmi', "blank", "NORTH-1,WEST+9", tooltipTheme = "pod", desc = "Enable or disable the secondary system installed in the pod, if there is one")
		lock = create_screen("lock", "Lock", 'icons/ui/hud_pod.dmi', "lock-locked", "NORTH-1,WEST+10", tooltipTheme = "pod-alt", desc = "LOCK YOUR PODS YOU DOOFUSES")
		set_code = create_screen("set_code", "Set Lock code", 'icons/ui/hud_pod.dmi', "set-code", "NORTH-1,WEST+11", tooltipTheme = "pod", desc = "Set the code used to unlock the pod")
		rts = create_screen("return_to_station", "Return To [capitalize(station_or_ship())]", 'icons/ui/hud_pod.dmi', "return-to-station", "NORTH-1,WEST+12", tooltipTheme = "pod", desc = "Using this will place you on the station Z-level the next time you fly off the edge of the current level")
		leave = create_screen("leave", "Leave Pod", 'icons/ui/hud_pod.dmi', "leave", "SOUTH,EAST", tooltipTheme = "pod-alt", desc = "Get out of the pod")
		rcs = create_screen("rcs", "Toggle RCS", 'icons/ui/hud_pod.dmi', "rcs-off", "NORTH-1,WEST+13", tooltipTheme = "pod-alt", desc = "Reduce the pod's relative velocity")
		tracking = create_screen("tracking", "Tracking Indicator", 'icons/ui/hud_pod.dmi', "off", "CENTER, CENTER")
		tracking.mouse_opacity = 0
		sensor_lock = create_screen("sensor_lock", "Sensor Lock", 'icons/ui/hud_pod.dmi', "off", "SOUTH+1,EAST")
	...
	relay_click(id, mob/user, list/params)
		...
		switch (id)
			...
			if ("return_to_station")
				if(UNDERWATER_MAP)
					boutput(user, "[master.ship_message("INCOMPATIBLE SPACIAL MEDIUM")]")
				else
					if(master.com_system)
						if(master.com_system.active)
							master.going_home = 1
