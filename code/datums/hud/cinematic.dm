		...
		switch (name)
			if ("nuke")
				...
				SPAWN_DBG(3.5 SECONDS)//45)
					...
					del(src)*/
			if ("gehenna_nuke")
				var/atom/movable/screen/hud/anim = create_screen("cinematic", "", 'icons/effects/gehenna_explosion.dmi', "start_nuke", "1:6, 1:50", 99)
				clients << sound('sound/misc/airraid_loop_short.ogg')
				SPAWN_DBG(3.5 SECONDS)//45)
					anim.icon_state = "explode"
					sleep(1 SECOND)
					clients << sound('sound/voice/farts/superfart.ogg')
					sleep(8 SECONDS)
					anim.icon_state = "loss_nuke"
			if ("malf")
