...
//[00:45]	AngriestIBM	Let me generate a round backstory in 5 seconds: This is how the syndicate determines who gets higher ranking positions -- by handing the candidates quad-injectors of insurgents and having them fight to the death
...
/datum/game_mode/spy/pre_setup()
	...
	var/i = rand(5)
	var/num_teams = max(setup_min_teams, min(floor((num_players + i) / 7), setup_max_teams))
...
/datum/game_mode/spy/proc/equip_leader(mob/living/carbon/human/leader)
	...
	//equip_traitor(leader) <- Quad insurgents and the starter gear are more than sufficient. Spies really don't need a traitor uplink on top of that.
	...
	boutput(leader, "<span class='notice'>You've been supplied with a <b>special quad-use implanter</b> in the spy starter kit in your [!isnull(the_slot) ? "[the_slot]" : "UNKNOWN"]. Use it to recruit some henchmen!</span>")
	return
...
/datum/game_mode/spy/proc/add_spy(mob/living/new_spy, mob/living/leader)
	...
	src.spies[spymind] = leadermind
	spymind.special_role = "spyrecruit"
