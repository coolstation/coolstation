...
/obj/banshark/
	...
	proc/banproc()
		...
		if(sharktarget2)
			...
			logTheThing("admin", callster:client, data["ckey"], "has evaded the shark by ceasing to exist!  Banning them anyway.")
	...
	proc/gibproc()
		...
		if (IN_RANGE(src, src.sharktarget2, 1))
			...
			if(sharktarget2?.client)
				...
				message_admins("<span class='internal'>[callster?.client?.ckey] has sharkgibbed [sharktarget2.ckey].</span>")
				sharktarget2.gib()
			sleep(0.5 SECONDS)
			playsound(src.loc, pick('sound/voice/burp_alien.ogg'), 50, 0)
			sleep(0.5 SECONDS)
