...
/obj/machinery/abcu
	...
	process()
		...
		for (var/obj/item in src)
			...
			if (istype(item, /obj/item/sheet))
				...
				if (src.metal_owed && sheets.material.material_flags & MATERIAL_METAL)
					...
					continue
				if (src.crystal_owed && sheets.material.material_flags & MATERIAL_CRYSTAL)
				...
				if (src.metal_owed && bars.material.material_flags & MATERIAL_METAL)
					...
					continue
				if (src.crystal_owed && bars.material.material_flags & MATERIAL_CRYSTAL)
	...
	proc/prepare_build(mob/user)
		if(src.invalid_count)
			boutput(usr, "<span class='alert'>The machine cannot build on dense turfs. Check for red markers.</span>")
			return
		...
		logTheThing("station", src, "[user] started ABCU build at [log_loc(src)], with blueprint [src.current_bp.room_name], authored by [src.current_bp.author]")
	...
	proc/get_blueprint(mob/user, var/savepath = "")
		...
		if (load?.room_name)
			src.current_bp = load
			logTheThing("station", user, "[user] loaded blueprint [load.room_name] (authored by: [load.author]) in [src].") // no slurs kthx
...
/verb/adminDeleteBlueprint()
	...
	if (!deleted) return
	logTheThing("admin", usr, "[usr] deleted blueprint [deleted["file"]], owned by [deleted["ckey"]].")
...
/obj/item/abcu_blueprint_reference
	...
	attack_self(mob/user)
		...
		var/input = ckeyEx(text_input(user, "You are copying '[src.room_name]' to your own collection. \
		...
		while (input && fexists("data/blueprints/[user.client.ckey]/[input].dat"))
			if (!user?.client?.ckey || timeout > 5)
				...
				return
			input = ckeyEx(text_input(user, "A blueprint named '[input]' already exists. Please input another, or cancel.",
...
proc/save_abcu_blueprint(mob/user, list/turf_list, var/use_whitelist = TRUE)
	...
	// ckeyEx to sanitize filename: no spaces/special chars, only '_', '-', and '@' allowed. 54 char limit in tgui_input
	var/input = strip_html(text_input(user, "Set a name for your new blueprint. \
...
/obj/item/blueprint_marker
	...
	pixelaction(atom/target, params, mob/user)
		...
		if(abs(minx - maxx) >= maxSize || abs(miny - maxy) >= maxSize)
			...
			playsound(src.loc, 'sound/machines/bweep.ogg', 25)
			return
		...
		switch (selecting)
			...
			if (SELECT_FIRST_CORNER, DESELECT_FIRST_CORNER) // set to 1 or 2 by use-in-hand option list
				...
				user << corner1img
				playsound(src.loc, 'sound/machines/ping.ogg', 15)
				return
		...
		if(roomList.Find(target))
			...
			playsound(src.loc, 'sound/machines/ping.ogg', 25, 0.1)
		else
			roomList.Add(target)
			roomList[target] = image('icons/misc/old_or_unused.dmi',target,"tiletag", layer = HUD_LAYER)
			updateOverlays()
			playsound(src.loc, 'sound/machines/ping.ogg', 15, 0.1)
	...
	attack_self(mob/user as mob)
		...
		switch(input)
			...
			if("Share A Blueprint")
				...
				logTheThing("station", user, "[user] printed [printed], named '[picked["roomname"]]' (authored by: [picked["author"]]).")
				return
...
/obj/item/blueprint_marker/verb/migrate_bigfile_blueprint()
	...
	// ckeyEx to sanitize filename: no spaces/special chars, only '_', '-', and '@' allowed. 54 char limit in tgui_input
	var/new_save_name = strip_html(text_input(user, "Set a name for your migrated blueprint. \
		Filename conversion preserves only alphanumeric characters, and - and _.",
