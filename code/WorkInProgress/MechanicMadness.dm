	...
	receive_signal(datum/signal/signal)
		...
		if((only_directed && signal.data["address_1"] == src.net_id) || !only_directed || (signal.data["address_1"] == "ping"))
			...
			if(signal.encryption)
				...
				SEND_SIGNAL(src, COMSIG_MECHCOMP_TRANSMIT_SIGNAL, html_decode("[signal.encryption]" + stars(packets, signal.encryption_visibility)), null)
				animate_flash_color_fill(src,"#ff0000",2, 2)
				return
	...
	proc/fire(var/datum/mechanicsMessage/input)
		...
		if(input && Gun)
			if(Gun.canshoot())
				...
				if(target)
					Gun.Shoot(get_turf(target), get_turf(src), src)
			else
