...
/mob/proc/DoGPS(var/ID)
	...
	var/list/paths = get_path_to(OT, landmarks[LANDMARK_GPS_WAYPOINT], max_distance=120, id=ID, skip_first=FALSE, cardinal_only=FALSE, do_doorcheck = TRUE)
...
/mob/living/carbon/verb/GPS()
	...
	if(ON_COOLDOWN(src, "gps", 10 SECONDS) && !src.client.holder)
