...
/obj/machinery/navbeacon
	...
	var/beacon_id = ""		// beacon_id response text
	var/list/codes		// assoc. list of transponder codes
	/// these two variables are different to allow packethacking to work without massive amounts of pings while pathfinding
	var/list/neighbors = list() // assoc. list of netids as key and distance as value
	...
	proc/neighbors(var/max_dist = 150)
		var/list/obj/machinery/navbeacon/found_neighbors = list()
		for(var/netid in src.neighbors)
			if(text2num(src.neighbors[netid]) <= max_dist)
				for_by_tcl(other_navbeacon, /obj/machinery/navbeacon)
					if(netid == other_navbeacon.net_id)
						found_neighbors += other_navbeacon
	...
	receive_signal(datum/signal/signal)
		...
		if(!signal.data["sender"]) // no sender, ignore
			return

		if(signal.data["sender"] == src.net_id) // no talking to ourself
			return

		if (signal.data["command"] == "neighborbeacons" && signal.data["beacon"] && signal.data["x"] && signal.data["y"] && signal.data["z"])
			if(src.codes[signal.data["beacon"]])
				return
			var/turf/my_turf = get_turf(src)
			var/turf/neighbor_turf = locate(text2num_safe(signal.data["x"]), text2num_safe(signal.data["y"]), text2num_safe(signal.data["z"]))
			if (my_turf && neighbor_turf && my_turf.z == neighbor_turf.z)
				var/neighbor_distance = GET_EUCLIDEAN_DIST(my_turf, neighbor_turf)
				if (neighbor_distance <= 40)
					src.neighbors[signal.data["sender"]] = "[neighbor_distance]"
					post_distance_update(signal.data["sender"], neighbor_distance)
			return

		if (!signal.data["address_1"]) // no target, ignore
			return
		...
		switch (signal.data["command"])
			if ("help")
				...
				if (!signal.data["topic"])
					...
					reply.data["topics"] = "status,set_beacon_id,set_code"
				else
					...
					switch (lowertext(signal.data["topic"]))
						if ("status")
							reply.data["description"] = {"Returns the status of the nav beacon. This includes the beacon
								beacon_id and all of the configurable transponder codes.
								Please consult your internal documentation for information about these codes"}
						if ("set_beacon_id")
							reply.data["description"] = "Sets the beacon beacon_id name"
							reply.data["args"] = "beacon_id"
						if ("set_code")
			...
			if ("set_beacon_id")
				if (!signal.data["beacon_id"]) return
				var/newloq = adminscrub(signal.data["beacon_id"])
				src.beacon_id = newloq
				post_status(signal.data["sender"])
			if ("set_code")
			...
			if ("set_neighbor")
				...
				var/code_key = adminscrub(signal.data["code_key"])
				var/code_value = adminscrub(signal.data["code_value"])
				neighbors.Remove(code_key)
				var/turf/my_turf = get_turf(src)
				if(text2num_safe(z) == my_turf.z && code_value)
					neighbors[code_key] = code_value
				post_status(signal.data["sender"])

	proc/post_distance_request()
		var/datum/radio_frequency/frequency = radio_controller.return_frequency("[freq]")
		...
		signal.data["beacon"] = beacon_id
		signal.data["netid"] = net_id
		signal.data["command"] = "neighborbeacons"
	...
	proc/post_distance_update(var/target, var/distance)
		if(!target || !distance)
			return
		...
		signal.data["command"] = "set_neighbor"
		signal.data["code_key"] = net_id
		if (distance)
			signal.data["code_value"] = "[distance]"
		var/turf/T = get_turf(src)
		signal.data["x"] = "[T.x]"
		signal.data["y"] = "[T.y]"
		signal.data["z"] = "[T.z]"
	...
	// return a signal giving beacon_id and transponder codes
...
<HR>
Beacon ID: <A href='byond://?src=\ref[src];locedit=1'>[beacon_id ? beacon_id : "(none)"]</A><BR>
Transponder Codes:<UL>"}
		...
		if ((in_interact_range(src, usr) && istype(src.loc, /turf)) || (issilicon(usr)))
			if(open && !locked)
				...
				if (href_list["freq"])
					...
					var/newloc = input("Enter New Beacon ID", "Navigation Beacon", beacon_id) as text|null
...
/obj/machinery/navbeacon/mule
	...
	hallway_arrivals_north
		...
		west
			codes_txt = "delivery;dir=8"
	hallway_fore_north
		beacon_id = "North Primary Hallway"
		codes_txt = "delivery;dir=1"
	...
	hallway_starboard_north
		beacon_id = "East Primary Hallway"
		codes_txt = "delivery;dir=1"
		...
		west
			codes_txt = "delivery;dir=8"
	hallway_aft_north
		beacon_id = "South Primary Hallway"
		codes_txt = "delivery;dir=1"
		...
		west
			codes_txt = "delivery;dir=8"
	hallway_port_north
		beacon_id = "West Primary Hallway"
		codes_txt = "delivery;dir=1"
	...
	hallway_central_north
		beacon_id = "Central Primary Hallway"
		codes_txt = "delivery;dir=1"
...
/obj/machinery/navbeacon/tour/cog1
	...
	tour16
		...
		beacon_id = "tour16"
		codes_txt = "tour;next_tour=tour17;desc=In the event of catastrophic station damage or approved shift change, a shuttle will be dispatched from the regional Central Command dock to this beacon_id. Should this occur, remember to line up quietly and enter single-file. No shoving please!"
...
/obj/machinery/navbeacon/tour/oshan
	tour0
		name = "tour beacon - 'Arrivals'"
		beacon_id = "tour0"
		codes_txt = "tour;next_tour=tour1;desc=Hello friend, and welcome to to the Nanotrasen Deep Sea Facility Tour! I will be your tourguide today on this adventure! Let's get started!\nThis lovely beacon_id is the arrivals lobby! It supports the PRISMA arrival pod system, which provides employees with their own private and efficient shuttle transport. Where do they launch from? Well, that's classified."
...
/obj/machinery/navbeacon/tour/chunk
	...
	tour7
		...
		beacon_id = "tour7"
		codes_txt = "tour;next_tour=tour8;desc=Okay! Here's the medical bay in case you get hurt. Probably won't. But please memorize this beacon_id immediately. Oh also there's the hydroponics bay just across the hall! Yeah!"
