...
/obj/machinery/ghostdrone_factory
	...
	process(mult)
		...
		if (working && src.current_assembly)
			...
			if (prob(40))
				SPAWN_DBG(0)
					src.shake(rand(4,6))
				playsound(src, pick("sound/impact_sounds/Wood_Hit_1.ogg", "sound/impact_sounds/Metal_Hit_Heavy_1.ogg"), 30, 1, SOUND_RANGE_STANDARD)
			if (prob(40))
	...
	proc/force_new_drone()
		var/obj/item/ghostdrone_assembly/G = new()
