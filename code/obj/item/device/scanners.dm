...
/obj/item/device/t_scanner
	...
	afterattack(atom/A as mob|obj|turf|area, mob/user as mob)
		if (istype(A, /turf))
			...
			if ((M.status & MALFUNC) && src.on)
				animate_scanning(A, "#7693d3")
				user.visible_message("<span class='alert'><b>[user]</b> has scanned the [A].</span>")
				boutput(user, "<br><span class='alert'>Maintenance analysis: <b>[M] is not functioning properly.</span></b>")
				src.audible_message("<span class='notice'>The NanoTrasen maintenance guide suggests: <i>[M.malfunction_hint()]</i></span>")
			else if((M in random_events.maintenance_event.unmaintained_machines) && src.on)
				animate_scanning(A, "#7693d3")
				user.visible_message("<span class='alert'><b>[user]</b> has scanned the [A].</span>")
				boutput(user, "<br><span class='alert'>Maintenance analysis: [M] is overdue for maintenance.</span>")
				src.audible_message("<span class='notice'>The NanoTrasen maintenance guide suggests: <i>[M.malfunction_hint()]</i></span>")
		if (istype(A, /obj) && A.interesting)
			animate_scanning(A, "#7693d3")
			user.visible_message("<span class='alert'><b>[user]</b> has scanned the [A].</span>")
