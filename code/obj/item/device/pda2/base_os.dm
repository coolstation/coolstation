			...
			switch(src.mode)
				...
				if(MODE_ADDRESSBOOK) // Specific names sent to us, also ringtones
					...
					if(length(src.all_callers) < 1)
						...
						. += "<table cellspacing=5>"
						for(var/call_source in src.all_callers)
							var/muteButton = "<a href='byond://?src=\ref[src];manageBlock=["add"];type=["single"];entry=[src.all_callers[call_source]]'>Block</a>"
							var/callButton = "<a href='byond://?src=\ref[src];input=message;target=[call_source]'>Msg</a>"
							var/sendButton = "<a href='byond://?src=\ref[src];input=send_file;target=[call_source]'>Send File</a>"
							if(!src.master.fileshare_program)
								sendButton = ""
							else if(!src.clipboard || src.clipboard?.dont_copy)
								sendButton = "<strike>Send File</strike>"
							var/delButton = "<a href='byond://?src=\ref[src];delAddress=[call_source]'>Del</a>"
							if(src.all_callers[call_source] in src.blocked_numbers)
								muteButton = "<a href='byond://?src=\ref[src];manageBlock=["remove"];type=["single"];entry=[src.all_callers[call_source]]'>Unblock</a>"
							. += "<tr><td>[src.all_callers[call_source]]</td><td>[callButton]</td><td>[muteButton]</td><td>[sendButton]</td><td>[delButton]</td></tr>"
						. += "</table>"
					. += "<hr>"
