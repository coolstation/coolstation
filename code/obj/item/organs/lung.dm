...
/obj/item/organ/lung
	...
	on_transplant(var/mob/M as mob)
		..()
		if (src.robotic)
			APPLY_ATOM_PROPERTY(src.donor, PROP_STAMINA_REGEN_BONUS, icon_state, 2)
	...
	on_removal()
		...
		if (donor)
			if (src.robotic)
				REMOVE_ATOM_PROPERTY(src.donor, PROP_STAMINA_REGEN_BONUS, icon_state)
	...
	proc/breathe(datum/gas_mixture/breath, underwater, mult, datum/organ/lung/status/update)
		...
		if (CO2_pp > safe_co2_max)
			if (!donor.co2level) // If it's the first breath with too much CO2 in it, lets start a counter, then have them pass out after 12s or so.
				donor.co2level = world.time
			else if (world.time - donor.co2level > 120)
				...
				if (world.time - donor.co2level > 300) // They've been in here 30s now, lets start to kill them for their own good!
...
/obj/item/organ/lung/cyber
	...
	disposing()
		if(donor)
			REMOVE_ATOM_PROPERTY(donor, PROP_REBREATHING, "cyberlungs")
		..()
