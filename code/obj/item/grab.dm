		...
		switch (src.state)
			if (GRAB_PASSIVE)
				if (src.affecting.buckled)
					...
					user.next_click = world.time + user.combat_click_delay  * GET_ATOM_PROPERTY(user, PROP_COMBAT_CLICK_DELAY_SCALING) //+ rand(6,11) //this was utterly disgusting, leaving it here in memorial
			if (GRAB_AGGRESSIVE)
				if (ishuman(src.affecting))
				...
				user.next_click = world.time + user.combat_click_delay * GET_ATOM_PROPERTY(user, PROP_COMBAT_CLICK_DELAY_SCALING)
