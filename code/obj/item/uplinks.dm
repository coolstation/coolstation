...
/obj/item/uplink
	...
	New()
		..()
		SPAWN_DBG(1 SECOND)
			if (src && istype(src) && (!length(src.items_weapons) && !length(src.items_utility) && !length(src.items_personal) && !length(src.items_telecrystal)))
				src.setup()
	...
	proc/setup(var/datum/mind/ownermind, var/obj/item/device/master)
		...
		if (!islist(src.items_weapons))
			src.items_weapons = list()
		if (!islist(src.items_utility))
		...
		for (var/datum/syndicate_buylist/S in syndi_buylist_cache)
			if (src.is_VR_uplink)
				...
				if (S.objective)
					...
					src.items_utility.Add(S)
				else
					src.items_weapons.Add(S)
				...
				if (istype(S, /datum/syndicate_buylist/weapon) && !src.items_weapons.Find(S))
					src.items_weapons.Add(S)

				if (istype(S, /datum/syndicate_buylist/utility) && !src.items_utility.Find(S))
					if (S.telecrystal)
						src.items_telecrystal.Add(S)
						src.items_utility.Remove(S)
					else
						src.items_utility.Add(S)

				if (ownermind || istype(ownermind))
					if (ownermind.special_role != ROLE_NUKEOP && istype(S, /datum/syndicate_buylist/traitor))
						if (!S.objective && !S.job && !src.items_weapons.Find(S))
							src.items_weapons.Add(S)
						...
						if (ownermind.objectives)
							...
							for (var/datum/objective/O in ownermind.objectives)
								if (istype(O, S.objective))
									has_objective = 1
							if (has_objective && !src.items_personal.Find(S))
								src.items_personal.Add(S)
		...
		if (length(src.items_weapons))
			var/list/sort1 = list()

			for (var/datum/syndicate_buylist/S1 in src.items_weapons)
				var/name = S1.name
			...
			src.items_weapons = sortList(sort1)

		if (length(src.items_utility))
			var/list/sort2 = list()

			for (var/datum/syndicate_buylist/S2 in src.items_utility)
				var/name = S2.name
				if (name in names)
					namecounts[name]++
					name = text("[] ([])", name, namecounts[name])
			...
			src.items_utility = sortList(sort2)

		if (length(src.items_personal))
			var/list/sort3 = list()

			for (var/datum/syndicate_buylist/S3 in src.items_personal)
				var/name = S3.name
				if (name in names)
					namecounts[name]++
					name = text("[] ([])", name, namecounts[name])
		...
		if (src.selfdestruct)
			...
			if (src.temp)
				...
				if (src.items_telecrystal && islist(src.items_telecrystal) && length(src.items_telecrystal))
					dat += "</table><B>Miscellaneous:</B><BR><table cellspacing=5>"
				...
				if (src.items_personal && islist(src.items_personal) && length(src.items_personal))
					dat += "</table><B>Agent specific:</B><BR><table cellspacing=5>"
				...
				if (src.items_utility && islist(src.items_utility) && length(src.items_utility))
					...
					for (var/J in src.items_utility)
						var/datum/syndicate_buylist/I2 = src.items_utility[J]
						dat += "<tr><td><A href='byond://?src=\ref[src];spawn=\ref[src.items_utility[J]]'>[I2.name]</A> ([I2.cost])</td><td><A href='byond://?src=\ref[src];about=\ref[src.items_utility[J]]'>About</A></td>"
				if (src.items_weapons && islist(src.items_weapons) && length(src.items_weapons))
					dat += "</table><B>Weapons:</B><BR><table cellspacing=5>"
					for (var/G in src.items_weapons)
						var/datum/syndicate_buylist/I1 = src.items_weapons[G]
						dat += "<tr><td><A href='byond://?src=\ref[src];spawn=\ref[src.items_weapons[G]]'>[I1.name]</A> ([I1.cost])</td><td><A href='byond://?src=\ref[src];about=\ref[src.items_weapons[G]]'>About</A></td>"
				dat += "</table>"
				var/do_divider = 1
	...
	proc/validate_spawn(var/datum/syndicate_buylist/SB)
		...
		for(var/S in items_weapons)
			if(SB == items_weapons[S])
				return 1

		for(var/S in items_utility)
			if(SB == items_utility[S])
				return 1
...
/obj/item/uplink/integrated/pda
	...
	generate_menu()
		...
		if (src.items_weapons && islist(src.items_weapons) && length(src.items_weapons))
			src.menu_message += "</table><B>Syndicate Weapons:</B><BR><table cellspacing=5>"
			for (var/G in src.items_weapons)
				var/datum/syndicate_buylist/I1 = src.items_weapons[G]
				src.menu_message += "<tr><td><A href='byond://?src=\ref[src];buy_item=\ref[src.items_weapons[G]]'>[I1.name]</A> ([I1.cost])</td><td><A href='byond://?src=\ref[src];abt_item=\ref[src.items_weapons[G]]'>About</A></td>"
		if (src.items_utility && islist(src.items_utility) && length(src.items_utility))
			src.menu_message += "</table><B>Utility Items:</B><BR><table cellspacing=5>"
			for (var/J in src.items_utility)
				var/datum/syndicate_buylist/I2 = src.items_utility[J]
				src.menu_message += "<tr><td><A href='byond://?src=\ref[src];buy_item=\ref[src.items_utility[J]]'>[I2.name]</A> ([I2.cost])</td><td><A href='byond://?src=\ref[src];abt_item=\ref[src.items_utility[J]]'>About</A></td>"
		if (src.items_personal && islist(src.items_personal) && length(src.items_personal))
			src.menu_message += "</table><B>Agent specific:</B><BR><table cellspacing=5>"
			for (var/O in src.items_personal)
				var/datum/syndicate_buylist/I3 = src.items_personal[O]
