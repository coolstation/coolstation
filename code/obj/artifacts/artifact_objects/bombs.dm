...
/datum/artifact/bomb
	...
	effect_activate(var/obj/O)
		...
		if (alarm_initial)
			playsound(T, alarm_initial, 100, 1, doAlert?250:SOUND_RANGE_STANDARD)
		if (doAlert)
	...
	effect_process(var/obj/O)
		...
		if(TIME > src.detonation_time)
			...
			if (alarm_final)
				playsound(T, alarm_final, 100, 1, SOUND_RANGE_STANDARD)
...
/datum/artifact/bomb/blackhole
	...
	deploy_payload(var/obj/O)
		if (..())
			return
		var/turf/T = get_turf(O)
		playsound(T, "sound/machines/satcrash.ogg", 100, 0, SOUND_RANGE_STANDARD, 0.8)
