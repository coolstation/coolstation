...
/datum/targetable/hide_between_floors
	...
	cast(atom/T)
		...
		if(M.layer == BETWEEN_FLOORS_LAYER)
			...
			APPLY_ATOM_PROPERTY(M, PROP_CANTMOVE, "floorswitching")
			REMOVE_ATOM_PROPERTY(M, PROP_NO_MOVEMENT_PUFFS, "floorswitching")
			REMOVE_ATOM_PROPERTY(M, PROP_NEVER_DENSE, "floorswitching")
			...
			SPAWN_DBG(0.4 SECONDS)
				if(M)
					...
					REMOVE_ATOM_PROPERTY(M, PROP_CANTMOVE, "floorswitching")
				if(floorturf?.intact)
			...
			SPAWN_DBG(0.4 SECONDS)
				if(M)
					REMOVE_ATOM_PROPERTY(M, PROP_CANTMOVE, "floorswitching")
					APPLY_ATOM_PROPERTY(M, PROP_NO_MOVEMENT_PUFFS, "floorswitching")
					APPLY_ATOM_PROPERTY(M, PROP_NEVER_DENSE, "floorswitching")
					M.flags |= NODRIFT | DOORPASS | TABLEPASS
					M.set_density(0)
...
/datum/targetable/ankle_bite
	...
	cast(atom/target)
		...
		if(holder.owner.layer == BETWEEN_FLOORS_LAYER)
			animate_slide(floorturf, x_coeff * -slide_amount, y_coeff * -slide_amount, 4)
			APPLY_ATOM_PROPERTY(holder.owner, PROP_CANTMOVE, "floorbiting")
			SPAWN_DBG(0.4 SECONDS)
				if(holder.owner && target_human && IN_RANGE(holder.owner, target, 1))
					...
					REMOVE_ATOM_PROPERTY(holder.owner, PROP_CANTMOVE, "floorbiting")
				else
