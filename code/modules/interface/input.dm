...
/client
	...
	*/
	...
	Click(atom/object, location, control, params)
		...
		if ((isnum(next) && next > 0) && src.preferences.use_click_buffer && src.queued_click != object && next <= max(user.click_delay, user.combat_click_delay) * GET_COMBAT_CLICK_DELAY_SCALE(user))
