...
#define SERVICE_JSON_PARAM_ADMINONLY "admine_only"
...
/datum/tgs_api/v3210/proc/ListServiceCustomCommands(warnings_only)
	...
	for(var/I in typesof(/datum/tgs_chat_command) - /datum/tgs_chat_command)
		...
		if(!warnings_only)
			.[command_name] = list(SERVICE_JSON_PARAM_HELPTEXT = initial(stc.help_text), SERVICE_JSON_PARAM_ADMINONLY = initial(stc.admine_only), SERVICE_JSON_PARAM_REQUIREDPARAMETERS = 0)
