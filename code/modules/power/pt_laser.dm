...
/obj/machinery/power/pt_laser/proc/updateicon(var/started_firing = 0)
	...
	if(status & BROKEN || charge == 0)
		overlays += image('icons/obj/machines/pt_laser.dmi', "unpowered")
		return

	if(load_last_tick > 0)
		overlays += image('icons/obj/machines/pt_laser.dmi', "green_light")

	if(online)
		overlays += image('icons/obj/machines/pt_laser.dmi', "red_light")
		if(started_firing)
			overlays += image('icons/obj/machines/pt_laser.dmi', "started_firing")
		else if(firing)
			overlays += image('icons/obj/machines/pt_laser.dmi', "firing")

	var/clevel = chargedisplay()
	if(clevel == 6)
		overlays += image('icons/obj/machines/pt_laser.dmi', "charge_full")
	else if(clevel>0)
		overlays += image('icons/obj/machines/pt_laser.dmi', "charge_[clevel]")
...
/obj/proc/burn_living(var/mob/living/L,var/power = 0)
	...
	if(prob(min(power/1e5,50)))
		INVOKE_ASYNC(L, /mob/livingPROC_REF(emote, "scream")) //might be spammy if they stand in it for ages, idk
