	...
	proc/classic_grump(mult)
		...
		switch (lastgenlev)
			if(0)
				...
				return
			if(1 to 2)
				playsound(src.loc, sound_engine1, 50, 0, pitch = 0.8, forcechannel = src.engine_loop_channel)
				if(prob(5))
					playsound(src.loc, pick(sounds_engine), 70, 0)
			if(3 to 7)
				playsound(src.loc, sound_engine1, 55, 0, pitch = 0.9, forcechannel = src.engine_loop_channel)
			if(8 to 11)
				playsound(src.loc, sound_engine1, 55, 0, pitch = 1, forcechannel = src.engine_loop_channel)
			if(12 to 15)
				playsound(src.loc, sound_engine2, 55, 0, pitch = 0.8, forcechannel = src.engine_loop_channel)
			if(16 to 18)
				playsound(src.loc, sound_bellalert, 70, 0)
				playsound(src.loc, sound_engine2, 60, 0, pitch = 0.9, forcechannel = src.engine_loop_channel)
				if (prob(5))
					elecflash(src, power = 3)
			if(19 to 21)
				playsound(src.loc, sound_warningbuzzer, 50, 0)
				playsound(src.loc, sound_engine2, 60, 0, pitch = 1, forcechannel = src.engine_loop_channel)
				if (probmult(5))
			...
			if(22 to 23)
				...
				playsound(src.loc, sound_engine2, 60, 0, pitch = 1.02, forcechannel = src.engine_loop_channel)
			...
			if(24 to 25)
				playsound(src.loc, sound_engine_alert1, 55, 0)
				playsound(src.loc, sound_engine2, 60, 0, pitch = 1.05, forcechannel = src.engine_loop_channel)
			...
			if(26 to INFINITY)
				...
				playsound(src.loc, sound_engine2, 45, 0, pitch = (1.1), forcechannel = src.engine_loop_channel)
