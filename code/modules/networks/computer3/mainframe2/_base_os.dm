	...
	receive_progsignal(var/sendid, var/list/data, var/datum/computer/file/file)
		...
		switch (data["command"])
			...
			if (DWAINE_COMMAND_ULOGIN)
				...
				var/datum/computer/file/mainframe_program/call_source = master.processing[sendid]
				if (!call_source || !data["name"])
					return ESIG_GENERIC

				if (data["name"] == "TEMP" && data["data"])
					return (login_temp_user(data["data"], null, call_source)) ? ESIG_GENERIC : ESIG_SUCCESS

				else
					if (!call_source.useracc)
						return ESIG_NOUSR

					if (login_user(call_source.useracc, data["name"], (data["sysop"] == 1), (data["service"] != 1)))
						return ESIG_GENERIC

				return ESIG_SUCCESS

			if (DWAINE_COMMAND_UGROUP) //Manipulate the group of the call_source's user account to ["group"]
				if (!sendid)
					return ESIG_GENERIC

				var/datum/computer/file/mainframe_program/call_source = master.processing[sendid]
				if (!call_source || !isnum(data["group"]))
					return ESIG_GENERIC

				if (!call_source.useracc || !call_source.useracc.user_file)
					return ESIG_NOUSR

				call_source.useracc.user_file.fields["group"] = min(255, max(data["group"], 0))
				return ESIG_SUCCESS
			...
			if (DWAINE_COMMAND_UMSG) //Send message to a user terminal, and ONLY user terminals (In contrast with msg_term)
				...
				var/datum/computer/file/mainframe_program/call_source = master.processing[sendid]
				if (!call_source || !call_source.useracc)
					return ESIG_NOUSR

				if (call_source.useracc == target)
					return ESIG_NOTARGET

				var/senderName = call_source.useracc.user_name
				if (!senderName)
					return ESIG_NOUSR
			...
			if (DWAINE_COMMAND_TSPAWN) //Spawn task
				...
				task_model = src.master.run_program(task_model, (pass_user ? call_source.useracc : null), call_source, data["args"])
				if (!task_model)
					return ESIG_GENERIC
			...
			if (DWAINE_COMMAND_TFORK)
				...
				var/datum/computer/file/mainframe_program/fork = src.master.run_program(call_source, null, call_source, data["args"], 1)
				if (!fork)
					return ESIG_GENERIC
			...
			if (DWAINE_COMMAND_TKILL) //Kill a child task of the calling program.
				...
				if (target_task.parent_task != call_source)
					return ESIG_GENERIC

				var/datum/mainframe2_user_data/target_user = target_task.useracc

				if (target_user && (!call_source.useracc || target_user.current_prog == target_task))
					target_user.current_prog = call_source
					call_source.useracc = target_user
			...
			if (DWAINE_COMMAND_TLIST) //List all child tasks of the calling program.
				...
				for (var/x = 1, x <= master.processing.len, x++)
					...
					if (MP && MP.parent_task == call_source)
						.[x] = MP
					else
						.[x] = null
			...
			if (DWAINE_COMMAND_FGET) //Return the computer datum at the provided path, if it exists.
				...
				var/datum/computer/file/mainframe_program/call_source = master.processing[sendid]
				if (!data["path"] || !call_source)
					return ESIG_NOTARGET

				. = parse_datum_directory(data["path"], src.holder.root, 0, call_source.useracc)
				if (.)
			...
			if (DWAINE_COMMAND_FKILL) //Delete the computer datum at the provided path, if possible.
				...
				var/datum/mainframe2_user_data/the_user = call_source.useracc
...
//					qdel(F)
			...
			if (DWAINE_COMMAND_FMODE) //Adjust the permissions of the file at the provided path, if possible.
				...
				var/datum/computer/target_datum = parse_datum_directory(data["path"], src.holder.root, 0, call_source.useracc)
				if (!istype(target_datum))
					return ESIG_NOFILE

				if (call_source.useracc && !check_mode_permission(target_datum, call_source.useracc))
					return ESIG_GENERIC
