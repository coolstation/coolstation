...
/obj/machinery/bot/mining/process()
	...
	if(!istype(target, /turf/wall/asteroid/))
		src.target = null
	if(!src.target)
		src.findTarget()
	if(!src.target)
...
/obj/machinery/bot/mining/proc/findTarget()
	...
	for (var/turf/wall/asteroid/D in view(7,src))
...
/datum/action/bar/icon/digbotdig
	...
	var/turf/wall/asteroid/target

	New(var/obj/machinery/bot/mining/bot, var/turf/wall/asteroid/target)
		..()
		src.bot = bot
		src.target = target
	...
	proc/checkStillValid()
		if(bot == null || target == null)
			...
			return false
		if(!bot.on || !istype(target, /turf/wall/asteroid/))
			bot.target = null
			interrupt(INTERRUPT_ALWAYS)
			return false
		return true
