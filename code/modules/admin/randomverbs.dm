...
/client/proc/cmd_admin_advview()
	...
	if (!adventure_view || mob.see_invisible < 21)
		adventure_view = 1
		mob.see_invisible = INVIS_ADVENTURE + 1
		...
		if (!isliving(mob))
			mob.see_invisible = INVIS_WRAITH // this seems to be quasi-standard for dead and wraith mobs? might fuck up target observers but WHO CARES
		else
