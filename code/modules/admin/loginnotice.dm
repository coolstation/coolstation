...
/datum/admins/proc/setLoginNotice(var/target_key)
	...
	if (!new_message && message)
		if (!player.cloud_put("login_notice", null))
			alert(src.owner.mob, "ERROR: Failed to clear login notice to cloud for [target_key].")
			return
	...
	alert(src.owner.mob, "Login notice for '[target_key]' has been set. They should see it next time they connect.")
		...
		var/login_notice_html = {"
						...
						<html>
							...
							<body>
								...
								<p>If you need to talk with an admin, please <a href="byond://winset?command=adminhelp">Adminhelp</a> or post on the <a href="https://forums.coolstation.space/">forums</a>.</strong></p>
								<hr>
...
/client/proc/acknowledge_login_notice()
	...
	if (message)
		if (!player.cloud_put("login_notice", null))
			alert(src.mob, "ERROR: Failed to clear login notice for some reason...")
			return
