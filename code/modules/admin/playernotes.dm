...
/datum/admins/proc/viewPlayerNotes(var/player)
	...
	var/deletelinkpre = "<A href='byond://?src=\ref[src];action=notes2;target=[player];type=del;id="
	var/deletelinkpost = "'>(DEL)</A>"

	var/regex/R = new("!!ID(\\d+)", "g")
	content = R.Replace(content, "[deletelinkpre]$1[deletelinkpost]")

	var/dat = "<h1>Player Notes for <b>[player]</b></h1><HR><br><A href='byond://?src=\ref[src];action=notes2;target=[player];type=add'>Add Note</A><br><HR>"
