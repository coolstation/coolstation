...
/mob/living/carbon/wall/horror
	say_quote(var/text)
		if(src.emote_allowed)
			...
			if(src.gender == MALE) playsound(src, "sound/voice/screams/male_scream.ogg", 100, 0, SOUND_RANGE_STANDARD, 0.91, channel=VOLUME_CHANNEL_EMOTE)
			else playsound(src, "sound/voice/screams/female_scream.ogg", 100, 0, SOUND_RANGE_STANDARD, 0.9, channel=VOLUME_CHANNEL_EMOTE)
			SPAWN_DBG(5 SECONDS)
				src.emote_allowed = 1
			return "screams!"
		else
	...
	attack_self(var/mob/user as mob)
		...
		switch(pick)
			...
			if("ONION SLUG CANDY") // Anagram: ANNOYING CLOUDS
				...
				playsound(user, 'sound/ambience/nature/Wind_Cold2.ogg', 50, 1, SOUND_RANGE_LARGE)
