	...
	New(var/loc,var/lifespan = 2.5 MINUTES)
		...
		for (var/obj/machinery/interdictor/IX in by_type[/obj/machinery/interdictor])
			if (IN_RANGE(IX,src,IX.interdict_range) && IX.expend_interdict(9001))
				playsound(IX,'sound/machines/alarm_a.ogg',50,0,SOUND_RANGE_STANDARD,1.5)
				SPAWN_DBG(3 SECONDS)
					if(IX) playsound(IX,'sound/machines/alarm_a.ogg',50,0,SOUND_RANGE_STANDARD,1.5)
		...
		for (var/mob/M in range(14,src))
			...
			shake_camera(M, 5, 16)
		playsound(src,'sound/effects/creaking_metal1.ogg',100,0,SOUND_RANGE_LARGE,0.5)

		sleep(lifespan / 2)
		if (!stable)
			src.visible_message("<span class='alert'><b>[src] begins to collapse in on itself!</b></span>")
			playsound(src,'sound/machines/engine_alert3.ogg',100,0,SOUND_RANGE_LARGE,0.5)
		...
		if (!stable)
			...
			playsound(src,'sound/machines/satcrash.ogg',100,0,SOUND_RANGE_LARGE,0.5)
