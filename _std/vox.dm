...
/*
 * Did a slight reorg and cleanup as commenting practice.
 * Should be a bit easier if you ever need to wade into the voxes again.
 * Vox Procs and Lists for both the expansive NabaVox and the classic HLVox
 *
 * Admins get to use this as an audio intercom.
 * Admins can also turn this on as a fun toy for the AI.
 *
 * Every sound is cached, please make sure it's rolled up into soundcache if you ever make any changes.
 * Every sound is also categorized for some mad libs thing but I'm not even sure where that is.
 * The hellmess of that is now at the very end of the file, after all the actually useful and legible stuff.
 *
 * Credits:	-SpaceNaba did NabaVox audio
 * 			-Valve did HLVox audio
 * 			-A whole lot of sorry suckers did all the manual categorization and addition to the datum list.
 */

/* -------------------------- Defines for handling -------------------------- */
...
var/global/
	...
	// The Price Master is a kind of vox right?
...
/* ---------------------- Helper datums for vox sounds ---------------------- */

/datum/VOXsound
	var/id
	var/ogg
	var/sound/soundFile
	var/flags //What kind of word or speech is this
...
/datum/hlvoxsound
	...
	proc/load(var/freq = 1)
		if (src.ogg)
			src.soundFile = sound(src.ogg, wait = 1, channel = 5)
			src.soundFile.frequency = freq
			src.soundFile.volume = 50 //HLVox is normalized differently than Nabavox so we can tweak this
...
/* ------------------------- Announcement - Nabavox ------------------------- */

// Prompt for a string of sounds
...
// Prompt for a string of sounds but also apply a specified pitch shift to them
...
// Build the sentence and play it to all clients
...
// Check and build or rebuild soundlist as necessary
...
// Filter for player-controlled vox (i.e. a.i.)
...
proc/hlvox_play(var/input, var/user, var/pitch = 1)
	...
	//loop through all words, determine pitch, and build the soundfile in memory
	//TODO: currently this fucks up a lot and will play out of order
	var/list/soundQueue = list()
...
// Build and display a list of all valid words
/client/proc/cmd_admin_intercom_help()
	SET_ADMIN_CAT(ADMIN_CAT_FUN)
	set name = "Intercom Help (Wordlist)"
...
/client/proc/cmd_admin_hlvox_help()
	SET_ADMIN_CAT(ADMIN_CAT_FUN)
	set name = "HLVOX Help (Wordlist)"
	set desc = "ASS BLAST USA"
...
// Show what tokens and sounds actually got loaded up
...
/* -------------------------------------------------------------------------- */
/*                          The Big Hellmess of Words                         */
/* -------------------------------------------------------------------------- */

/*
 * Global init procs are called from world.New so that's how this gets blasted into the game
 * This feels like a candidate for reading from a file but what do I know
 * There's some handling at the end of these godawful lists in the init proc so take a look down there at the SPAWN_DBGs if something's busted
*/

// Historical record found carved into this .dm:
...
// i'm sorry to report that i have no idea if it did fix the things they intended to fix but other things are still total busted ass. in here it remains. rip
