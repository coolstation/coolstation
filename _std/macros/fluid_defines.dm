...
//same as above but including pool check
#define IS_VALID_FLUIDREACT_TURF(T) (IS_VALID_FLUID_TURF(T) && !(locate(/obj/pool) in src))
...
#define MAX_VISCOSITY 20
#define viscosity_SLOW_COMPONENT(avg_viscosity, max_speed_mod) (((avg_viscosity-1)/(MAX_VISCOSITY-1)) * max_speed_mod * 0.5)
